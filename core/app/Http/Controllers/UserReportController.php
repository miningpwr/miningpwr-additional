...
class UserReportController extends Controller
{
    ...
    public function roiLog(Request $request)
    {
        $search = $request->search;

        if ($search) {
            $data['page_title'] = "ROI search : " . $search;
            $data['transactions'] = auth()->user()->transactions()->where('remark', 'everyday_roi')->where('trx', 'like', "%$search%")->latest()->paginate(getPaginate());
        } else {
            $data['page_title'] = "ROI Log";
            $data['transactions'] = auth()->user()->transactions()->where('remark', 'everyday_roi')->with('date')->latest()->paginate(getPaginate());
        }
        $data['search'] = $search;
        $data['empty_message'] = "No Data Found!";
        return view($this->activeTemplate . 'user.roiLog', $data);
    }

    public function levelLog(Request $request)
    {
        $search = $request->search;

        if ($search) {
            $data['page_title'] = "Level search : " . $search;
            $data['transactions'] = auth()->user()->transactions()->where('remark', 'level_bonus')->where('trx', 'like', "%$search%")->latest()->paginate(getPaginate());
        } else {
            $data['page_title'] = "Level Log";
            $data['transactions'] = auth()->user()->transactions()->where('remark', 'level_bonus')->latest()->paginate(getPaginate());
        }
        $data['search'] = $search;
        $data['empty_message'] = "No Data Found!";
        return view($this->activeTemplate . 'user.levelLog', $data);
    }
