...
@section('panel')
    ...
    <div class="card">
        <div class="row text-center justify-content-center llll" data-parent="1">
            <div class="w-1" data-count="1">
                @php echo showSingleUserinTree($tree['a']); @endphp
            </div>
        </div>
        <div class="row text-center justify-content-center llll" data-parent="2">
            <div class="w-2" data-count="1">
                @php echo showSingleUserinTree($tree['b']); @endphp
            </div>
            <div class="w-2 " data-count="2">
                @php echo showSingleUserinTree($tree['c']); @endphp
            </div>
        </div>
        <div class="row text-center justify-content-center llll" data-parent="3">
            <div class="w-4 " data-count="1">
                @php echo showSingleUserinTree($tree['d']); @endphp
            </div>
            <div class="w-4 " data-count="2">
                @php echo showSingleUserinTree($tree['e']); @endphp
            </div>
            <div class="w-4 " data-count="3">
                @php echo showSingleUserinTree($tree['f']); @endphp
            </div>
            <div class="w-4 " data-count="4">
                @php echo showSingleUserinTree($tree['g']); @endphp
            </div>

        </div>
        <div class="row text-center justify-content-center llll" data-parent="4">
            <div class="w-8" data-count="1">
                @php echo showSingleUserinTree($tree['h']); @endphp
            </div>
            <div class="w-8" data-count="2">
                @php echo showSingleUserinTree($tree['i']); @endphp
            </div>
            <div class="w-8" data-count="3">
                @php echo showSingleUserinTree($tree['j']); @endphp
            </div>
            <div class="w-8" data-count="4">
                @php echo showSingleUserinTree($tree['k']); @endphp
            </div>
            <div class="w-8" data-count="5">
                @php echo showSingleUserinTree($tree['l']); @endphp
            </div>
            <div class="w-8" data-count="6">
                @php echo showSingleUserinTree($tree['m']); @endphp
            </div>
            <div class="w-8" data-count="7">
                @php echo showSingleUserinTree($tree['n']); @endphp
            </div>
            <div class="w-8" data-count="8">
                    ...
                    <div class="user-details-modal">
                        ...
                        <div class="user-details-body text-center">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <td>@lang('Total Paid')</td>
                                    <td colspan="2" class="text-center"><span class="paid"></span></td>
                                </tr>
                            </table>
...
@push('script')
        ...
        (function ($) {
            $(document).ready(function(){
                ...
                $('.openLink').on('click', function () {
                    window.location.assign($(this).data('treeurl'));
                });
            });
    ...
    function handleClick(e) {
        ...
        jQuery(function($) {
            var count = $(e.parentNode).data('count');
            var parent = $(e.parentNode).parent().attr('data-parent');
            
            var referer = $("[data-parent='"+(parent-1)+"']").find("[data-count='"+(Math.round(count/2))+"'] .user-name").html();
            if(referer != "Add New"){
                var position = (count % 2 == 0)?"Right":"Left";
                window.location.assign("{{ route('user.addUser') }}"+"?username="+referer+"&position="+position);            
                
            }else{
                alert("Fill the parent first!");
            }
        }); 
    }
