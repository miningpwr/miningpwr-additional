...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        $data['openWallet']         = Transaction::where(['user_id' => auth()->id(), 'trx_type' => '+', 'wallet_type' => 'open_wallet'])->sum('amount') - Transaction::where(['user_id' => auth()->id(), 'trx_type' => '-', 'wallet_type' => 'open_wallet'])->sum('amount');
        $data['savingWallet']       = Transaction::where(['user_id' => auth()->id(), 'trx_type' => '+', 'wallet_type' => 'saving_wallet'])->sum('amount') - Transaction::where(['user_id' => auth()->id(), 'trx_type' => '-', 'wallet_type' => 'saving_wallet'])->sum('amount');
        $data['cashWallet']       = Transaction::where(['user_id' => auth()->id(), 'trx_type' => '+', 'wallet_type' => 'cash_wallet'])->sum('amount') - Transaction::where(['user_id' => auth()->id(), 'trx_type' => '-', 'wallet_type' => 'cash_wallet'])->sum('amount');
    ...
    public function withdrawMoney()
    {
        ...
        //     $notify[] = ['error', 'You can withdraw only in SUNDAY in a week. '];
        //     return back()->withNotify($notify);
        // }
        return view(activeTemplate() . 'user.withdraw.methods', $data);
    }
     public function recognization()
    {
        //$data['withdrawMethod'] = WithdrawMethod::whereStatus(1)->get();
        $data['page_title'] = "Recognization";
        return view(activeTemplate() . 'user.recognization', $data);
    }
    ...
    public function withdrawStore(Request $request)
    {
        ...
        $openWallet = Transaction::where(['user_id' => auth()->id(), 'trx_type' => '+', 'wallet_type' => 'open_wallet'])->sum('amount') - Transaction::where(['user_id' => auth()->id(), 'trx_type' => '-', 'wallet_type' => 'open_wallet'])->sum('amount');
        ...
        // $totalBalance = ($openWallet  $cashWallet);
        if($openWallet > $cashWallet){
            $totalBalance = $openWallet;
            $wallet = "open_wallet";
        }else if($openWallet <= $cashWallet){
            $totalBalance = $cashWallet;
            $wallet = "cash_wallet";
        }
        ...
        if (Carbon::today()->format('l') != "Monday") {
            $notify[] = ['error', 'You can withdraw only in MONDAY in a week. '];
            return back()->withNotify($notify);
        }
    ...
    public function withdrawPreview()
    {
        ...
        // $totalBalance = ($openWallet  $cashWallet);
        if($openWallet > $cashWallet){
            $data['mainBalance'] = $openWallet;
            $data['walletType'] = "Open wallet";
        }else if($openWallet <= $cashWallet){
            $data['mainBalance'] = $cashWallet;
            $data['walletType'] = "Cash wallet";
        }
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        // if($withdraw->final_amount <= $cashWallet){
        //     $transaction->wallet_type =  "cash_wallet";
        // }else{
    ...
    function balanceTransfer(Request $request)
    {
        ...
        if ($cashWallet >= $amount) {
            ...
            Transaction::create([
                ...
                'post_balance' => getAmount($cashWallet - $amount),
                'charge' => $charge,
            ...
            $trans_user->save();
            $cashWallet = Transaction::where(['user_id' => $trans_user->id, 'trx_type' => '+', 'wallet_type' => 'cash_wallet'])->sum('amount') - Transaction::where(['user_id' => $trans_user->id, 'trx_type' => '-', 'wallet_type' => 'cash_wallet'])->sum('amount');
    ...
    public function showAddUser()
    {
        ...
        return view($this->activeTemplate.'user.addUser', compact('page_title', 'content', 'country_code'));
    }
