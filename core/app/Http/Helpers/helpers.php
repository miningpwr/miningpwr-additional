...
function showSingleUserinTree($user)
{
    $res = '';
    if ($user) {
        session()->put("last_id", $user->id);
        if ($user->plan_id == 0) {
            $userType = "free-user bg-black";
            $stShow = "Free";
            $planName = '';
        } else {
            $planName = $user->plan->name;
            $userType = "paid-user bg-".strtolower($planName);
            $stShow = "";
        }

        // $img = getImage('assets/images/user/profile/'. $user->image, '120x120');
        $img = asset('assets/templates/basic/users/images/ic_user.png');
        if($user->image){
        ...
        $extraData .= " data-paid=\"" . getAmount(@$user->total_binary_com) . "$\"";
        $extraData .= " data-lfree=\"" . getAmount(@$user->left_active) . "$\"";
        $extraData .= " data-rfree=\"" . getAmount(@$user->right_active) . "$\"";
        $extraData .= " data-lbv=\"" . getAmount(@$user->left_count) . "$\"";
        $extraData .= " data-rbv=\"" . getAmount(@$user->right_count) . "$\"";
        
        $linkData = "data-treeurl=\"$hisTree\"";

        $res .= "<div class=\"user\" type=\"button\">";
        $res .= "<img src=\"$img\" alt=\"*\"  class=\"$userType openLink\" $linkData> <i class=\"fa fa-info-circle showDetails position-absolute\" style='top:80%' $extraData></i>";
        $res .= "<p class=\"user-name text-dark\">$user->username </p>"; 
        // $res .= "<div class=\"d-flex justify-content-around\" onclick=\"showDetails(this)\" $extraData ><p class=\"user-name\">$user->username </p>"; 

// $res .= "<p class=\"user-name\">$user->username <i class=\"showDetails\" type=\"button\"  $extraData><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"16\" height=\"16\" fill=\"currentColor\" class=\"bi bi-info-circle\" viewBox=\"0 0 16 16\">
//   <path d=\"M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z\"/>
//   <path d=\"m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533L8.93 6.588zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0z\"/>
// </svg></i> </p>";
        // $res .= "<i class=\"fa fa-info-circle \" ></i> </div>";

    } else {
        $position = 0;
        if(session()->has('last_id')){
            session()->put("last_last_id", session('last_id'));
            session()->forget('last_id');
            $position = 1;
        }else if(session()->has('last_last_id')){
            session()->forget('last_last_id');
            $position = 2;
        }
        // $data = "onClick=\"(function(){window.location.assign('".route('user.addUser')."?position={$position}');}())\"";
        $data = "onClick='handleClick(this)'";
        $img = asset('assets/templates/basic/users/images/ic_add_user.png');

        $res .= "<div class=\"user\" type='button' ".$data.">";
        $res .= "<img src=\"$img\" alt=\"*\"  class=\"no-user\">";
        $res .= "<p class=\"user-name text-success\">Add New</p>";
    }
...
// function find_position_id($placement_id){

//         $user = User::where('pos_id',$placement_id)->first();
//         if(!$user){
//             return null;
//         }
...
//             return null;
//         }
//         // print_r($user_id->pos_id);
//         return $user->pos_id;
// }
function find_placement_user($placement_id){
    return User::where('id',$placement_id)->first();
}
